...
class MushafDataService {
  ...
  Future<void> initDatabases() async {
    ...
    // Hafs database for words/scripts
    final wordsDbPath = p.join(databasesPath, 'hafs-script.db');
    if (!await File(wordsDbPath).exists()) {
      final wordsData =
          await rootBundle.load('assets/quran/renderer/hafs/script.db');
    ...
    // Hafs layout database
    final layoutDbPath = p.join(databasesPath, 'hafs-layout.db');
    if (!await File(layoutDbPath).exists()) {
      final layoutData = await rootBundle
          .load('assets/quran/renderer/hafs/layout.db');
  ...
  Future<void> _loadSurahNameFont() async {
    ...
    try {
      ...
      final fontData =
          await rootBundle.load('assets/quran/renderer/hafs/font.ttf');
